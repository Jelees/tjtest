...
//	}
	...
	public static void menuGuestInfo(String name, Integer mobile) {
		int a = String.valueOf(mUser.getPhone()).length();
		System.out.println("현재 고객 정보 : ");
		System.out.println("이름 " + mUser.getName() + "   연락처 " + String.format("%0"+ (a+1) +"d", mUser.getPhone()));
//		System.out.println("이름 " + mUser.getName() + "   연락처 " + mUser.getPhone());
	}
		...
		if (adminId.equals(admin.getId()) && adminPW.equals(admin.getPassword())) {
			...
			if (str.toUpperCase().equals("Y")) {
				...
				try {
					...
					//  FileWriter 클래스는 파일의 길이를 체크하는 메소드를 제공하지 않습니다.
					//  이를 대신할 수 있는 방법으로는, java.nio.file 패키지의 Files와 Paths 클래스를 이용하여야 합니다.

					FileWriter fw = new FileWriter("book.txt", true);

					// 새로운 파일에 기록하는 경우를 제외하고는 항상 줄바꿈을 먼저 실행한다.
					if(size > 0)
						fw.write("\n");
					// fw.write("\n");
					// FileWriter의 write() 메소드를 사용하여 파일에 쓸 때,
					// 자동으로 줄바꿈을 하지 않습니다.
					// 따라서, 이전에 파일에 쓴 내용 뒤에 바로 이어서 쓰게 됩니다.
					// 이럴 경우, 이전 도서의 마지막 정보와 새로운 도서의 첫 번째 정보(이 경우에는 ISBN)가 붙어서 저장됩니다.
					// 이 문제를 해결하기 위해서는, 새로운 도서 정보를 쓰기 전에 줄바꿈을 하면 됩니다.
					for (int i = 0; i < 6; i++)
						fw.write(writeBook[i] + "\n");
					fw.write(writeBook[6]);
					fw.close();
